...
async def get_data(asset):
    supply = await get_supply(asset["z_token"])
    debt = await get_debt(asset["underlying"])
    block_height = await client.get_block_number()
    return {
        "protocol": "zkLend",
        "date": get_today(),
        "market": asset["z_token"],
        ...
        "lending_index_rate": get_lending_accumulator(asset["underlying"])
    }
