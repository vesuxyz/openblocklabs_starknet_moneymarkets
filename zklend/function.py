...
async def get_debt_accumulator(underlying_address: int, block_number: int) -> int:
    ...
    (uint_value,) = await contract.functions["get_debt_accumulator"].call(underlying_address, block_number=block_number)
    return uint_value

# This function iterates through all the raw balances and debts per token for all users, 
# so it will take a while to run.
def get_raw_balance_per_user(underlying_address: str, block_number: int) -> Dict[str, Dict[str, int]]:
    ...
    while True:
        ...
        if response.status_code == 200:
            raw_supply_balances = (response.json())["data"]["ztokenRawBalances"]
            raw_debt_balances = (response.json())["data"]["userRawDebts"]
            ...
            for item in raw_supply_balances:
                ...
                if item["user"] in raw_balance_per_user:
                    ...
                    raw_balance_per_user[item["user"]] = {"supply": int(raw_supply), "debt": 0}
            for item in raw_debt_balances:
                raw_debt = item["amount"]
                if item["user"] in raw_balance_per_user:
                    raw_balance_per_user[item["user"]]["debt"] = int(raw_debt)
                else:
                    raw_balance_per_user[item["user"]] = {"supply": 0, "debt": int(raw_debt)}
...
def calc_non_recursive_supply(raw_balance_per_user: Dict[str, Dict[str, int]], lending_accumulator: int, debt_accumulator: int):
    total_non_recursive_supply = Decimal(0)
    for _, balances in raw_balance_per_user.items():
        uint_face_value_supply = Decimal(balances["supply"]) * Decimal(lending_accumulator)
...
async def get_data(asset):
    ...
    raw_balance_per_user = get_raw_balance_per_user(asset["underlying"], block_height)
    total_non_recursive_supply = calc_non_recursive_supply(raw_balance_per_user, lending_accumulator, debt_accumulator)
    ...
    return {
        ...
        "non_recursive_supply_token": scale_down(total_non_recursive_supply, asset["token_decimals"]),
